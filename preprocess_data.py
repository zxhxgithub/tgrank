...
def run(data_dir, data_name, bipartite=False, num_node_features=172):
  ...
  rand_feat = np.zeros((max_idx + 1, num_node_features))
...
parser.add_argument('--data', type=str, help='Dataset name (eg. reddit, wikipedia, mooc, lastfm)',
                    default='wikipedia')
parser.add_argument('--bipartite', action='store_true', help='Whether the graph is bipartite')
parser.add_argument('--num_node_feats', type=int, help='Number of random node features',
                    default=172)


args = parser.parse_args()

run(args.data_dir,args.data, bipartite=args.bipartite, num_node_features=args.num_node_feats)