...
from train_eval.training import listwise_training_epoch
...
def run(args):
    ...
    net = TSARNet(emb_dim=args.emb_dim, edge_attr_size=edge_features.shape[1], edge_time_emb_dim=args.time_dim, num_layers=args.num_layers, use_fourier_features=not args.no_fourier_time_encoding, use_id_label= not args.no_id_label, device=device).to(device)
    ...
    for epoch in range(args.num_epochs):
        ...
        # training
        loss, total_epoch_time = listwise_training_epoch(net, optimizer,train_data, full_data, node_features, edge_features, train_neighbor_finder, args.train_batch_size, args.num_temporal_hops, args.num_neighbors, args.verbose, args.coalesce_edges_and_time, args.train_randomize_timestamps)
        ...
        log.info('Trasductive Validation time:{}'.format(val_results["time"]))
        ...
        # Early stopping
        if early_stopper.early_stop_check(val_results["MRR"]):
